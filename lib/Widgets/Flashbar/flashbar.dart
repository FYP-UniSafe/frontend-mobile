...
import 'flashbar_route.dart' as route;

// ignore: constant_identifier_names
const String FLASHBAR_ROUTE_NAME = '/flashbarRoute';

typedef FlashbarStatusCallback = void Function(FlashbarStatus? status);
typedef OnTap = void Function(Flashbar flashbar);
...
class Flashbar<T> extends StatefulWidget {
  Flashbar(
      ...
      this.dismissDirection = FlashbarDismissDirection.VERTICAL,
      ...
      FlashbarStatusCallback? onStatusChanged,
      this.barBlur = 0.0,
      ...
      this.flashbarRoute,
      this.titleStyle,
      this.messageStyle // Please dont init this
      })
  ...
  final FlashbarStatusCallback? onStatusChanged;

  /// The title displayed to the user
  final String? title;

  /// The title text style displayed to the user.
  ...
  /// The message text style displayed to the user.
  ...
  /// Flushbar can be based on [FlashbarPosition.TOP] or on [FlashbarPosition.BOTTOM] of your screen.
  /// [FlashbarPosition.BOTTOM] is the default.
  final FlashbarPosition flashbarPosition;

  final double positionOffset;

  /// [FlashbarDismissDirection.VERTICAL] by default.
  /// Can also be [FlashbarDismissDirection.HORIZONTAL] in which case both left and right dismiss are allowed.
  final FlashbarDismissDirection dismissDirection;
  ...
  route.FlashbarRoute<T?>? flashbarRoute;

  /// Show the flushbar. Kicks in [FlashbarStatus.IS_APPEARING] state followed by [FlashbarStatus.SHOWING]
  Future<T?> show(BuildContext context) async {
    flashbarRoute = route.showFlashbar<T>(
      context: context,
      flashbar: this,
    ) as route.FlashbarRoute<T?>;

    return await Navigator.of(context, rootNavigator: false)
        .push(flashbarRoute as Route<T>);
  }
  ...
  Future<T?> dismiss([T? result]) async {
    ...
    if (flashbarRoute == null) {
      return null;
    }

    if (flashbarRoute!.isCurrent) {
      flashbarRoute!.navigator!.pop(result);
      return flashbarRoute!.completed;
    } else if (flashbarRoute!.isActive) {
      ...
      flashbarRoute!.navigator!.removeRoute(flashbarRoute!);
    }
  ...
  bool isShowing() {
    ...
    return flashbarRoute!.currentStatus == FlashbarStatus.SHOWING;
  }
  ...
  bool isDismissed() {
    ...
    return flashbarRoute!.currentStatus == FlashbarStatus.DISMISSED;
  }
  ...
  bool isAppearing() {
    ...
    return flashbarRoute!.currentStatus == FlashbarStatus.IS_APPEARING;
  }
  ...
  bool isHiding() {
    ...
    return flashbarRoute!.currentStatus == FlashbarStatus.IS_HIDING;
  }

  @override
  State createState() => _FlashbarState<T?>();
}

class _FlashbarState<K extends Object?> extends State<Flashbar<K>>
  ...
  Widget build(BuildContext context) {
    return Align(
      heightFactor: 1.0,
      child: Material(
        color: widget.flashbarStyle == FlashbarStyle.FLOATING
            ? Colors.transparent
  ...
  Widget _getFlashbar() {
    ...
    if (widget.userInputForm != null) {
      flashbar = _generateInputFlashbar();
    } else {
      flashbar = _generateFlashbar();
    }
...
/// If vertical, dismiss up will be allowed if [FlashbarPosition.TOP]
/// If vertical, dismiss down will be allowed if [FlashbarPosition.BOTTOM]
enum FlashbarDismissDirection { HORIZONTAL, VERTICAL }

/// Indicates the animation status
/// [FlashbarStatus.SHOWING] Flushbar has stopped and the user can see it
/// [FlashbarStatus.DISMISSED] Flushbar has finished its mission and returned any pending values
/// [FlashbarStatus.IS_APPEARING] Flushbar is moving towards [FlashbarStatus.SHOWING]
/// [FlashbarStatus.IS_HIDING] Flushbar is moving towards [] [FlashbarStatus.DISMISSED]
enum FlashbarStatus { SHOWING, DISMISSED, IS_APPEARING, IS_HIDING }
